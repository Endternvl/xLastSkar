...
const { Image_leave, message_leave } = require("../config");
...
client.on("guildMemberRemove", async (member) => {
  ...
  const background = await Canvas.loadImage(`${image || Image_leave}`);
  ...
  let c =
    (await client.data.get(`levmsg_${member.guild.id}`)) ||
    client.db.get(`levmsg_${member.guild.id}`) ||
    message_leave;
  let ch = c
    .split(`{member}`)
