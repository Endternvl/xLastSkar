...
module.exports = {
    name: 'gungame',
    description: 'A Gun Game! Credit = Face-hh',
    usage: '[USER]',
	cooldown: 5000,
    /**
     * @param { Client } client
    ...
    run: async (client, message, args) => {
        ...
        if (!opponent) return message.reply('Please enter a opponent to be matched!')
		const positions = {
			...
			ended1: '_ _     :levitate: :point_right:      **DIE!**        :skull_crossbones: :levitate:',
			ended2: '_ _     :levitate: :skull_crossbones:      **DIE!**        :point_left: :levitate:',
		};
		...
		const filter = button => {
			return button.user.id == message.author.id || button.user.id == opponent;
		};
		...
		if(button.customId === 'shoot1' && button.user.id == message.author.id) {
			...
			return button.reply({ embeds: [ new MessageEmbed({
                description: `**GG! <@${message.author.id}>** You Have Won The **PEW-PEW** War!`,
                color: 'GREEN' || 'BLURPLE' || 'LIGHT_GREY' || 'RED' || 'WHITE' || 'LUMINOUS_VIVID_PINK',
                footer: 'Thank you FACE!'
            }) ] });
		}
		else if(button.customId === 'shoot2' && button.user.id == opponent) {
			msg.edit({
				content: positions.ended1,
				components: componentsArray,
			});
			return button.reply({ embeds: [ new MessageEmbed({
                description: `**GG! ${opponent} ** You Have Won The **PEW-PEW** War!`,
                color: 'GREEN' || 'BLURPLE' || 'LIGHT_GREY' || 'RED' || 'WHITE' || 'LUMINOUS_VIVID_PINK',
                footer: 'Thank you FACE!'
            }) ] });
		}
